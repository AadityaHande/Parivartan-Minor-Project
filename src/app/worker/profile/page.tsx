...
export default function WorkerProfilePage() {
  ...
  // Use useWorkerProfile so we match by both workerId AND workerName — same
  // strategy as task/page.tsx and performance/page.tsx, avoiding fragile
  // displayName-only queries.
  ...
  const tasks = useMemo(() => {
    ...
    return allReports.filter(
      (r) =>
        isAssignedToWorker(r, workerId, workerName) &&
        (r.status === 'Resolved' || r.status === 'Rejected'),
    );
  }, [allReports, workerId, workerName]);

  const stats = useMemo(() => {
    if (!tasks.length) return { totalCompleted: 0, avgTime: 'N/A' };
  ...
  const isLoading = isUserLoading || isWorkerLoading || areReportsLoading;
